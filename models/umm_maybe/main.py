"""
umm-maybe/AI-image-detector wrapper (no reimplementation).
ViT-based image classifier (Oct 2022, VQGAN+CLIP era, artistic-image scope);
the base model Organika/sdxl-detector was fine-tuned from.
...
Label mapping (resolved at runtime from the published config's label2id,
then verified behaviorally — NOT assumed):
  "artificial" -> our "fake"
  "human"      -> our "real"
...
app = FastAPI(title="umm-maybe AI-Image Detector")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
MODEL_ID = os.getenv("UMM_MODEL_ID", "umm-maybe/AI-image-detector")
...
async def load_model():
    ...
    # NOTE: this config stores label2id values as STRINGS ("0"/"1") —
    # int() is required before tensor indexing (caught live on :5010).
    FAKE_INDEX = int(label2id[FAKE_LABEL])
...
async def detect(file: UploadFile = File(...)):
    ...
    try:
        ...
        logger.error(f"UMM_MAYBE detect error: {e}")
